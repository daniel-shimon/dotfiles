# tide + fzf config — re-applies customized universal vars.
# Run after fisher installs tide. Idempotent (set -U).
# Regenerate: see bootstrap notes.

# --- tide prompt layout ---
set -U _tide_left_items pwd git newline character
set -U _tide_right_items status cmd_duration jobs
...
# --- fzf custom settings (bindings live in config.fish) ---
